...
export class ModalBoletasMantenedorComponent implements OnInit {
  ...
  async generarContrato() {
    ...
    const contratosPorEstudiante = await Promise.all(estudiantes.map(async (estudiante: any) => {
      const cursoId = await this.getCursoId(estudiante.rut).toPromise();
      const apoderadoSuplente = await this.getInfoApoderadoSuplenteByRutEstudiante(estudiante.rut).toPromise();
  console.log(apoderadoSuplente);
      ...
      return {
        ...
        // valor_matricula: apoderadoScope.valor_matricula,
        // valor_mensualidad: apoderadoScope.valor_mensualidad,
